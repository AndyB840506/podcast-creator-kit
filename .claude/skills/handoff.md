...
```
Handoff created:
  .agents/handoff/2026-05-03-lead-generator-dashboard.md
  .agents/handoff/2026-05-03-lead-generator-dashboard-prompt.md

Session captured:
- 5 commits, 12 files changed
- Paused: mid-implementation of generate.php
- Next: Test PHP app in Laragon

To continue: Copy the prompt from lead-generator-dashboard-prompt.md
...
|---------|-------|----------|
| "No session activity found" | No commits, no file changes | Expected for idle sessions. Nothing to hand off. |
| Handoff files not written | `.agents/handoff/` directory missing | Run `mkdir -p .agents/handoff` |
| Topic slug is generic "session-1430" | No descriptive commits to derive from | Provide explicit topic: `/handoff lead-generator` |
| Post-mortem suggested but no learnings | Agent sees ≥3 commits and auto-suggests | Run `/post-mortem --quick` or skip if commits are trivial |
